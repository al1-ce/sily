...
void setTimeout(void delegate() func, int msecsTimeout) {
    spawn(cast(immutable) (&timeoutCallback), cast(immutable) func, cast(immutable) msecsTimeout);
}

private static void timeoutCallback(immutable void delegate() func, immutable int msecsTimeout) {
    Thread.sleep(msecsTimeout.msecs);
    func();
}
